...
describe('free-angle fixtures', () => {
  ...
  it('allows an exact 100k final grid plus bounded cascade overhead', () => {
    const result = searchFreeAngle(
      (angleDeg) => ({ angleDeg, ok: false }),
      {
        finalStepDeg: 0.0036,
    ...
    expect(result.evalCount).toBeGreaterThanOrEqual(100_000)
    expect(result.evalCount).toBeLessThan(100_100)
  })
})
