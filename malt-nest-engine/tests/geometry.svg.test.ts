...
describe('SVG input handling', () => {
  it('parses rounded rectangles with SVG radius defaults and clamping', () => {
    const rxOnly = parseSvg(svg('<rect width="10" height="6" rx="2"/>'))
    const ryOnly = parseSvg(svg('<rect width="10" height="6" ry="2"/>'))
    const explicit = parseSvg(
      svg('<rect width="10" height="6" rx="2" ry="2"/>'),
    )
    const clamped = parseSvg(
      svg('<rect x="1" y="2" width="10" height="4" rx="99" ry="99"/>'),
    )

    expect(rxOnly.meta.hasCurves).toBe(true)
    expect(ryOnly.shapes[0]!.polygons[0]!.outer).toEqual(
      rxOnly.shapes[0]!.polygons[0]!.outer,
    )
    expect(explicit.shapes[0]!.polygons[0]!.outer).toEqual(
      rxOnly.shapes[0]!.polygons[0]!.outer,
    )
    expect(shapeBounds(clamped.shapes[0]!)).toEqual({
      minX: 1,
      minY: 2,
      maxX: 11,
      maxY: 6,
    })
    expect(clamped.shapes[0]!.polygons[0]!.outer.length).toBeGreaterThan(4)
  })

  it('uses curve tolerance for rounded rectangles and rejects negative radii', () => {
    const markup = svg('<rect width="100" height="60" rx="20" ry="10"/>')
    const coarse = parseSvg(markup, {
      tolerance: { ...DEFAULT_TOLERANCE, curveTolerance: 2 },
    })
    const fine = parseSvg(markup, {
      tolerance: { ...DEFAULT_TOLERANCE, curveTolerance: 0.01 },
    })
    const sharp = parseSvg(svg('<rect width="10" height="6" rx="0" ry="2"/>'))

    expect(fine.shapes[0]!.polygons[0]!.outer.length).toBeGreaterThan(
      coarse.shapes[0]!.polygons[0]!.outer.length,
    )
    expect(sharp.meta.hasCurves).toBe(false)
    expect(sharp.shapes[0]!.polygons[0]!.outer).toHaveLength(4)
    expect(() =>
      parseSvg(svg('<rect width="10" height="6" rx="-1"/>')),
    ).toThrowError(/radius|rx/i)
    expect(() =>
      parseSvg(svg('<rect width="10" height="6" ry="-1"/>')),
    ).toThrowError(/radius|ry/i)
  })
