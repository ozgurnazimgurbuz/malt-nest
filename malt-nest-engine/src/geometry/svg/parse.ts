...
function roundedRectRing(
  ...
  const ring: { x: number; y: number }[] = [{ x: x + rx, y }]
  const appendLine = (point: { x: number; y: number }) => ring.push(point)
  const appendArc = (
    from: { x: number; y: number },
    to: { x: number; y: number },
  ) => ring.push(...flattenArc(from, rx, ry, 0, false, true, to, tol).slice(1))

  appendLine({ x: x + width - rx, y })
  appendArc(
    { x: x + width - rx, y },
    { x: x + width, y: y + ry },
  )
  appendLine({ x: x + width, y: y + height - ry })
  appendArc(
    { x: x + width, y: y + height - ry },
    { x: x + width - rx, y: y + height },
  )
  appendLine({ x: x + rx, y: y + height })
  appendArc(
    { x: x + rx, y: y + height },
    { x, y: y + height - ry },
  )
  appendLine({ x, y: y + ry })
  appendArc({ x, y: y + ry }, { x: x + rx, y })
  ring.pop()
  return ring
}
...
export function parseSvg(
  ...
  while (stack.length > 0) {
    ...
    if (tag === 'rect') {
      ...
      let rx = hasRx ? numAttr(el, 'rx') : undefined
      let ry = hasRy ? numAttr(el, 'ry') : undefined
      if ((rx ?? 0) < 0 || (ry ?? 0) < 0) {
        throw new Error('SVG rect radius (rx/ry) must be nonnegative')
      }
      ...
      if (w > 0 && h > 0) {
        ...
        const ring: Ring = rounded
          ? roundedRectRing(x, y, w, h, rx, ry, curveTolerance)
          : [
              { x, y },
              { x: x + w, y },
              { x: x + w, y: y + h },
