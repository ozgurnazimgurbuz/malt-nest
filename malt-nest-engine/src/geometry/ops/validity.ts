...
function collinearOverlapHasLength(
  ...
  const crossTolerance = tol.abs + tol.rel * length * Math.max(length, 1)
  if (Math.abs(cross0) > crossTolerance || Math.abs(cross1) > crossTolerance) {
    return false
  }
...
export function validateRing(
  ...
  if (!hasNonFiniteCoordinate) {
    if (nearlyZero(absoluteArea(effectiveRing), tol)) {
